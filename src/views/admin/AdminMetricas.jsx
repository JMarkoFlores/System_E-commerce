...
const AdminMetricas = ({ historialCompras, recomendaciones, estadisticasIA }) => {
  ...
  const datosRadar = metricas ? [
    { metrica: "Precisión", valor: parseFloat(metricas.precisionAtK), fullMark: 100 },
    { metrica: "Relevancia", valor: parseFloat(metricas.relevancia), fullMark: 100 },
    { metrica: "Diversidad", valor: parseFloat(metricas.diversidad), fullMark: 100 },
    { metrica: "Novedad", valor: parseFloat(metricas.novedad), fullMark: 100 },
    { metrica: "Cobertura", valor: parseFloat(metricas.cobertura), fullMark: 100 },
    { metrica: "Accuracy", valor: parseFloat(metricas.accuracy), fullMark: 100 },
  ] : [];

  const TarjetaMetrica = ({ icono: Icono, titulo, valor, maxValor = 100, color, descripcion, sufijo = "%" }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg p-6 text-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Activity size={40} />
            <div>
              <h2 className="text-3xl font-bold">Métricas de Evaluación del Modelo</h2>
              <p className="text-indigo-100">Análisis del rendimiento de la Red Neuronal con TensorFlow.js</p>
            </div>
          </div>
      ...
      <div className="bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg shadow-lg p-6 text-white">
        <h3 className="text-xl font-bold mb-4 flex items-center"><Brain className="mr-2" size={24} /> Estado del Modelo</h3>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white/20 backdrop-blur-sm rounded-lg p-4"><div className="text-sm opacity-90">Recomendaciones Activas</div><div className="text-3xl font-bold">{metricas.totalRecomendaciones}</div></div>
          <div className="bg-white/20 backdrop-blur-sm rounded-lg p-4"><div className="text-sm opacity-90">Aciertos Detectados</div><div className="text-3xl font-bold">{metricas.aciertos}</div></div>
          <div className="bg-white/20 backdrop-blur-sm rounded-lg p-4"><div className="text-sm opacity-90">Score Promedio</div><div className="text-3xl font-bold">{metricas.scorePromedio}%</div></div>
          <div className="bg-white/20 backdrop-blur-sm rounded-lg p-4"><div className="text-sm opacity-90">Confianza</div><div className="text-3xl font-bold">{metricas.confianza}%</div></div>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <TarjetaMetrica icono={Target} titulo="Precision@K" valor={metricas.precisionAtK} color="border-purple-600" descripcion="Relevancia de recomendaciones basada en categorías y tags" />
        <TarjetaMetrica icono={CheckCircle} titulo="Hit Rate" valor={metricas.hitRate} color="border-green-600" descripcion="Porcentaje de recomendaciones con score sobre el promedio" />
        <TarjetaMetrica icono={TrendingUp} titulo="Relevancia" valor={metricas.relevancia} color="border-blue-600" descripcion="Similitud con preferencias recientes" />
        <TarjetaMetrica icono={Award} titulo="Accuracy del Modelo" valor={metricas.accuracy} color="border-indigo-600" descripcion="Precisión general del modelo de IA" />
        <TarjetaMetrica icono={Zap} titulo="Diversidad" valor={metricas.diversidad} color="border-orange-600" descripcion="Variedad de categorías recomendadas" />
        <TarjetaMetrica icono={Activity} titulo="Novedad" valor={metricas.novedad} color="border-pink-600" descripcion="Productos fuera de la categoría favorita" />
      </div>
